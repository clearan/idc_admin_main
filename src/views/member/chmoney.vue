  ...
  >
    <div class="app-container">
      ...
      <div class="filter-container" style="margin-left: 60px">
        ...
        <el-input v-model="listQuery.val" clearable placeholder="手机号/会员账号" style="width: 250px;" class="filter-item" @keyup.enter.native="handleFilter" />
        ...
        methods:{
            ...
            handleFilter() {
                ...
                if (this.listQuery.val !== undefined && this.listQuery.val.length === 24) {
                    ...
                    data.mem_id = this.listQuery.val
                } else{
                ...
                this.$http.post(`${this.url}/get_memlist`,qs.stringify(data)).then((resp)=>{
                    ...
                    if (resp.ret_code === 0) {
                        if (resp.ret_data.data_list) {
                            ...
                            this.regist_time = formatDate(info['CreatedTime'])
                            this.balance = this.count(String(info['Amount']-info['Pledge']));
            ...
            onSubmit() {
                ...
                let data = {
                    ...
                    idempotent: new Date().getTime(),
                    id:this.id,
                    amount: this.type==1?balance:'-'+balance,
                    desc: this.remark,
                    operator: LocalStorage.get('user_name'),
                };
                this.$http.post(`${this.url}/mod_mem`,qs.stringify(data)).then((resp) => {
                    if (resp.ret_code >= 0) {
                        this.$message({
                            message:'金额更新成功',
                            type:'success',
